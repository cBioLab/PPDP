...
namespace PPDPP{
  ...
  int makePairVec(int turn,int cl,int sl,int threshold,std::vector< std::pair<int,int> >& cells,int &cells_len);
  ...
  public:
    ...
    void parallelDP(std::string& queryfile,std::string& resultfile,std::vector< std::pair<int,int> >& cells,int &cells_len);
  ...
  public:
    ...
    //必要なセルの位置をマップで確保.
    void makeQuerySet(std::string& query,std::vector< std::pair<int,int> >& cells,int &cells_len); 
    ...
    void decResultSet(std::string& result,std::vector< std::pair<int,int> >& cells,int &cells_len);
